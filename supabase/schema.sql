...
create table if not exists public.clients (
  ...
  preferred_communication_channel text not null default 'email' check (
    preferred_communication_channel in ('email', 'phone', 'sms', 'whatsapp')
  ),
...
create table if not exists public.jobs (
  ...
  status text not null default 'scheduled' check (
    status in ('scheduled', 'needs_confirmation', 'invite_sent', 'confirmed', 'completed', 'cancelled', 'reschedule_needed')
  ),
  google_calendar_event_id text,
  calendar_invite_status text not null default 'not_sent' check (
    calendar_invite_status in ('not_sent', 'needs_action', 'accepted', 'declined', 'tentative')
  ),
