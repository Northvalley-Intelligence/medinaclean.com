...
import { parseCrewUnavailabilityPayload } from "@/lib/crew-records";
...
export async function POST(request: Request) {
  ...
  const parsed = parseCrewUnavailabilityPayload(payload);
  ...
  try {
    await insertServiceRow("crew_unavailability", parsed.row);
  } catch (error) {
    console.error(error);
    return respond(
      request,
      { error: error instanceof Error ? error.message : "Unavailable time could not be saved.", lang },
      500,
      isForm
    );
  }
...
function respond(request: Request, body: Record<string, unknown>, status: number, isForm: boolean | undefined) {
  ...
  params.set(status >= 400 ? "error" : "unavailabilityCreated", String(body.error || "1"));
